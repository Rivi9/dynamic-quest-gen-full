...
The threshold values (0.75 and 1.30) that define the FLOW band are derived from Csikszentmihalyi's (\citeyear{csikszentmihalyi1990flow}) theoretical framework, in which Flow occupies a zone around the point of challenge--skill balance (ratio $= 1.0$), rather than the exact balance point itself. The asymmetry of the band ($-0.25$ below, $+0.30$ above) reflects observations from the DDA literature \citep{hunicke2004ai, yannakakis2009realtime}: players tolerate being slightly over-challenged (``productive struggle'') better than being under-challenged, and the subjective transition from engagement to boredom is sharper than the transition from engagement to anxiety. These thresholds are design parameters, not empirically calibrated values; a sensitivity analysis would be needed to establish optimal boundaries for a specific player population.
...
The observation vector includes four of the six Hexad dimensions; Free Spirit and Philanthropist are excluded. This is a deliberate dimensionality reduction. Free Spirit behaviour (unconstrained exploration uncorrelated with objectives) is largely captured by the Explorer dimension combined with a low challenge--skill ratio, making a separate dimension redundant for action selection. Philanthropist behaviour (deep lore and narrative engagement) is reflected in the dialogue engagement rate, which already enters the state indirectly through the Flow classifier's use of \texttt{dialogue\_engage\_rate}. Including all six dimensions would increase the state space without adding discriminative signal, slowing convergence during training. The four retained dimensions were selected on the basis of their orthogonality in the telemetry feature space and their direct relevance to the eight narrative actions.
...
The specific numerical values of these reward components are hand-tuned hyperparameters, not empirically derived quantities. The graduated severity ordering (APATHY $>$ ANXIETY $>$ BOREDOM) is grounded in the theoretical rationale that deeper disengagement states are harder to recover from \citep{csikszentmihalyi1990flow, chen2007flow}, but the exact magnitudes were determined through iterative experimentation during training: values were adjusted until the agent reliably converged on a non-degenerate policy that avoided repetitive action selection. The shaping rewards (streak bonus, transition bonus, repetition penalty) follow the general principle of potential-based reward shaping \citep{ng1999policy}, providing additional learning signal without altering the set of optimal policies under the base reward. A formal hyperparameter search would strengthen the justification and is identified as future work.
