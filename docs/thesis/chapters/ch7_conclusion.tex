...
Time management was a persistent problem. Integrating five modules across three technology stacks (Python, C\#, Ollama) took longer than I expected --- each interface worked in isolation but broke in combination. Weekly logbook entries helped, mainly because writing down what went wrong forced me to think about why.

Figure~\ref{fig:gantt} shows the project timeline as it actually unfolded.
...
\caption{Project Gantt chart (September 2025 -- March 2026). Major phases overlap deliberately: implementation began before design was finalised, and thesis writing ran alongside development. Milestones (red diamonds) mark proposal approval, system completion, and final submission.}
\label{fig:gantt}
\end{figure}
...
Whether the result is something players actually notice and value --- whether the story's attention registers as personalization rather than as noise --- is an empirical question that the evaluation study will begin to answer. But the harder questions are not empirical at all. What does it mean for a story to ``understand'' a player? Which behavioural signals are worth attending to, and which cross the line from personalization into surveillance? Is a system designed to sustain Flow --- to make the game harder to put down --- ethically equivalent to one designed to sustain revenue? These are design and ethics questions, not engineering questions, and this work does not pretend to resolve them. What it does provide is a concrete, working system against which they can be meaningfully asked.
