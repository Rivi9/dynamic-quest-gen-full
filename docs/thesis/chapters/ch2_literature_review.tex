...
\end{tikzpicture}
\caption{Literature map showing the six research domains reviewed in this chapter and their relationships. Solid arrows indicate contributions to the present work; dashed arrows indicate inter-domain dependencies.}
\label{fig:litmap}
\end{figure}
...
The original Hexad scale is a 24-item self-report questionnaire, validated for gamification contexts with acceptable internal consistency (Cronbach's $\alpha$ typically exceeding 0.70 per subscale). It has since been adopted across a range of application domains --- adaptive game design, recommendation systems, educational technology, health interventions --- establishing it as the de facto standard for research-grade player-type assessment \citep{hamari2014does}.
...
This dissertation addresses the limitation head-on by inferring Hexad-analogous continuous scores directly from behavioural telemetry, updating them continuously within the session. The mapping from telemetry signals to Hexad dimensions is a novel contribution, detailed in Section~3.3.
...
DDA works, and it has been deployed in commercial games ranging from racing titles (rubber-banding AI) to action games (adaptive enemy health pools). But the scope of what it adapts is narrow. DDA adjusts \textit{mechanical} parameters: how much damage enemies deal, how many spawn per wave, how generous the loot tables are. It does not adjust what the \textit{story does}. A player who is mechanically in the Flow zone but narratively disengaged --- trudging through a dungeon with no sense of purpose because the quest text failed to land --- is invisible to a DDA system.
